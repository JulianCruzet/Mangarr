...
class MangaDexProvider(MetadataProvider):
    ...
    def _parse_manga_data(manga_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        primary_title = MangaDexProvider._normalize_title(titles)
    ...
    async def search(
        ...
        results = [self._parse_manga_data(m) for m in data.get("data", [])]
    ...
    async def get_manga(self, provider_id: str) -> Optional[Dict[str, Any]]:
        ...
        return self._parse_manga_data(data.get("data", {}))
        ...
        async with httpx.AsyncClient(
            ...
            while True:
                async with _semaphore:
                    ...
                    resp = await client.get(f"/manga/{provider_id}/feed", params=params)
                    resp.raise_for_status()
                    data = resp.json()
    ...
    def _get_cover_url(self, manga_id: str, filename: str, size: int = 512) -> str:
    ...
    async def download_cover(
        ...
        url = self._get_cover_url(provider_id, cover_info, size=512)
            ...
            async with _semaphore:
                async with httpx.AsyncClient(
                    ...
                    if resp.status_code == 404:
                        ...
                        alt_url = f"{MANGADEX_UPLOADS_URL}/covers/{provider_id}/{cover_info}"
